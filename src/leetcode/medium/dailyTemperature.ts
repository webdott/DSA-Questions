...
const dailyTemperatures = (temperatures: number[]): number[] => {
	...
	for (let i = 0; i < temperatures.length; i++) {
		while (
			...
			monotonicStack[monotonicStack.length - 1] < temperatures[i]
		) {
			monotonicStack.pop();
			const prevDay: number = indexStack.pop()!;

			answer[prevDay] = i - prevDay;
		}

		monotonicStack.push(temperatures[i]);
		indexStack.push(i);
	}

	for (let remainingDay of indexStack) {
