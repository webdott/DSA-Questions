 ...
 * @param currTarget number
 * @param currIndex number
 * @param currArrs number[]
 * @param result number[][]
 * @returns
 */
const helper = (
	...
	if (currTarget === 0) {
		result.push(currArrs.slice());
		return;
	}

	while (
		currIndex < candidates.length &&
		currTarget - candidates[currIndex] >= 0
	) {
		currArrs.push(candidates[currIndex]);

		helper(
			candidates,
			currTarget - candidates[currIndex],
			currIndex,
			currArrs,
			result
		);
