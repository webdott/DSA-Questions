...
const nextGreaterElement = (nums1: number[], nums2: number[]): number[] => {
	...
	const monolithicStack: number[] = [-1];

	for (let i = nums2.length - 1; i >= 0; i--) {
		let found = false;
		while (monolithicStack.length > 0) {
			if (nums2[i] > monolithicStack[monolithicStack.length - 1]) {
				monolithicStack.pop();
			} else {
				hashTable.set(nums2[i], monolithicStack[monolithicStack.length - 1]);
				monolithicStack.push(nums2[i]);
				found = true;
				break;
			}
		}
	...
	const result = nums1.map((num) => hashTable.get(num));
	return result;
};
